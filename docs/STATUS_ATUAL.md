...
Ainda nao ha homologacao para producao. A camada OS continua saudavel: `systemctl --failed` permaneceu em `0 loaded units listed` e as rodadas nao mostraram `Oops`, `panic`, erro EXT4, remount read-only ou `mmc timeout/reset`. O bloqueio atual esta na camada app-MPV, especificamente responsividade de IPC, watchdog e coordenacao de restart. MPV via DRM/KMS continua sendo o caminho principal; Xorg, Wayland, compositor e Chromium continuam fora desta fase.
...
- Resolver a responsividade MPV IPC/watchdog/restart antes de liberar teste longo.
- Entender se restart por falha de ping IPC e destrutivo demais durante reproducao/transicao.
- Decidir politica final do watchdog apos A/B curto com reinicio por ping praticamente desativado.
...
1. Fazer A/B curto, ainda sem systemd, com `mpv_watchdog_ping_failures_before_restart=999`, mantendo timeout, `hwdec`, logging, midias, config operacional e coordenacao IPC/restart constantes.
2. Comparar a rodada contra `20260429-210204`: `Restarting MPV`, `MPV process started`, `MPV IPC command timeout`, `MPV IPC unresponsive`, `Failed to load media`, `Bad file descriptor`, status final, observacao visual e processos remanescentes.
3. So liberar teste manual mais longo se o A/B curto reduzir churn/restarts sem degradar exibicao, status final, kernel, systemd, escrita em `/opt` ou processos remanescentes.
