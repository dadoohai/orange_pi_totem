# C18 H2 Power-Loss Matrix Plan - c16fb3e
...
`rollback_after_current_unlinked` requires a custom lab setup with target as
current and no previous link/state. A normal target-over-previous rollback setup
must not be treated as sufficient for that checkpoint.
