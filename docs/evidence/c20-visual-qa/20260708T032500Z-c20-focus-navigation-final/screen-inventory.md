...
| wizard.orientation.step_focus | Mostrar foco unico no menu superior do wizard. | Enter abre | Baixo opcoes | Esc volta | low | yes |
| wizard.orientation.portrait | Confirmar orientacao em tela vertical. | Enter confirma | Esc volta | low | yes |
| wizard.network.choice | Escolher caminho de conexao. | Enter confirma | Esc cancela | medium | yes |
| wizard.wifi.empty | Listar, atualizar e selecionar rede Wi-Fi local. | Enter escolhe | R atualiza | Esc volta | low | yes |
| wizard.wifi.three_networks | Listar, atualizar e selecionar rede Wi-Fi local. | Enter escolhe | R atualiza | Esc volta | low | yes |
| wizard.wifi.eighteen_page_1 | Listar, atualizar e selecionar rede Wi-Fi local. | Enter escolhe | R atualiza | Esc volta | medium | yes |
| wizard.wifi.eighteen_page_2 | Listar, atualizar e selecionar rede Wi-Fi local. | Enter escolhe | R atualiza | Esc volta | medium | yes |
| wizard.wifi.eighteen_page_3 | Listar, atualizar e selecionar rede Wi-Fi local. | Enter escolhe | R atualiza | Esc volta | medium | yes |
| wizard.wifi.refreshing | Listar, atualizar e selecionar rede Wi-Fi local. | Enter escolhe | R atualiza | Esc volta | low | yes |
| wizard.wifi.scan_failure_cached | Listar, atualizar e selecionar rede Wi-Fi local. | Enter escolhe | R atualiza | Esc volta | low | yes |
| wizard.wifi.password_hidden | Coletar senha Wi-Fi no HDMI local. | Enter confirma | Esc volta | F2 alterna | low | yes |
| wizard.wifi.password_visible | Coletar senha Wi-Fi no HDMI local. | Enter confirma | Esc volta | F2 alterna | medium | yes |
| wizard.environment | Coletar identificador de ambiente. | Enter | Esc volta | low | yes |
| wizard.review | Revisar escolhas antes de gravar. | Enter | Cima menu | Esc volta | medium | yes |
| wizard.review_pending | Bloquear conclusao enquanto faltam etapas obrigatorias. | Enter | Cima menu | Esc volta | medium | yes |
...
| wifi_password_hidden | Digitar senha sem expor valor. | Enter confirma | Esc volta | F2 mostra/oculta. | low | yes |
| wifi_password_visible | Mostrar senha apenas no HDMI local. | Enter confirma | Esc volta | F2 oculta. | medium | yes |
| wifi_wrong_password | Explicar erro de senha ou associacao. | Enter tenta novamente | Esc volta para redes. | high | yes |
| weak_wifi | Avisar risco de sinal fraco. | Escolher TEST_WIFI_STRONG | R atualiza. | medium | yes |
| environment | Coletar ambiente sintetico. | Enter confirma | Esc volta. | medium | yes |
| review | Revisar dados sinteticos antes de salvar. | Enter salva | Esc volta. | low | yes |
