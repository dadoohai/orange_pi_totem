...
| H1.5 homologation pilot | Caminho intermediario controlado por `scripts/qa/c18_player_runtime_pilot_readiness_gate.py`: `channel=homologation`, `ring=pilot`, entrega assistida, autorizacao formal, preflight com public freeze `rc=44`, pacote alvo e P0 power-loss seletivo. O alvo corrente pre-P0 e `c18.player-runtime-homolog-20260617-mpv-stuck-fix-9bebaf1`; `docs/evidence/c18-update-validation/20260617T192801Z-pilot-preflight-mpv-stuck-fix-9bebaf1/pilot-readiness.json` esta bloqueado somente por `pilot_powerloss_p0`. A evidencia `20260616T232546Z-current-pilot-readiness-5b2128c`, com `20260612T195516Z-pilot-readiness-traceability-refresh-c16fb3e`, fica preservada como historico; apos a evidencia negativa `docs/evidence/c18-update-validation/20260617T174316Z-h2-powerloss-after-payload-staged-mpv-stuck-135f397/`, `c16fb3e` nao e mais RC corrente aceitavel para piloto. |
| Evidencia 1u | offline `20260608T024500Z-1u-offline-build`, cold-boot HW `20260608T035330Z-1u-coldboot-deep-health` |
| Power-loss fisico | O P0 seletivo historico do pacote `c16fb3e` cobria 5/17, mas a tentativa fisica `after_payload_staged` preservada em `20260617T174316Z-h2-powerloss-after-payload-staged-mpv-stuck-135f397` bloqueia esse alvo: MPV ficou em uma midia enquanto o status avancou. Para o alvo `9bebaf1`, a matriz foi reiniciada: `docs/evidence/c18-update-validation/20260617T193720Z-h2-powerloss-matrix-plan-mpv-stuck-fix-9bebaf1/` mostra 0/17 cobertos e 17/17 faltantes. O bloqueio inicial de topologia em `20260617T193921Z-h2-powerloss-board-preflight-mpv-stuck-fix-9bebaf1/` foi resolvido pelo reset governado `20260617T201430Z-h2-powerloss-topology-reset-mpv-stuck-fix-9bebaf1/`; o preflight fresco `20260617T201635Z-h2-powerloss-board-preflight-after-topology-reset-mpv-stuck-fix-9bebaf1/` e o refresh rastreavel `docs/evidence/c18-update-validation/20260617T203659Z-h2-powerloss-board-preflight-refresh-mpv-stuck-fix-9bebaf1/` foram aceitos para iniciar a sessao fisica, sem contar checkpoint nem liberar H2. |
...
| Rastreabilidade dos gates | Release gate decisivo git-guarda os diretorios coldboot/data; H1 atual `20260612T194911Z-1x-h1-decisive-traceability-refresh-7e40e80`; para o alvo `9bebaf1`, H2 readiness esta em `20260617T192101Z-current-h2-readiness-mpv-stuck-fix-9bebaf1` e continua vermelho somente por power-loss 17/17, soak 24h, stable promotion e decisao formal de thaw |
...
| Gate macro pre-H2 | `scripts/qa/c18_ota_macro_governance_gate.py` agrega H1, pilot readiness, H2 vermelho e diagnostico read-only em snapshot governado; nao substitui H2, nao reabre janela expirada e nao autoriza producao. O snapshot macro default antigo permanece historico de `c16fb3e`; o alvo `9bebaf1` ja tem preflight H2 aceito apos reset, mas ainda precisa de P0 antes de novo snapshot macro positivo. |
| Retomada operacional | `scripts/qa/c18_ota_operational_resume_gate.py` precisa ficar verde antes de qualquer nova execucao na placa; snapshots versionados de retomada provam apenas a janela em que foram coletados, e o gate default bloqueia sem autorizacao/preflight atuais e frescos |
