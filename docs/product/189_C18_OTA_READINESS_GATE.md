...
| Power-loss fisico | O P0 seletivo historico do pacote `c16fb3e` cobria 5/17, mas a tentativa fisica `after_payload_staged` preservada em `20260617T174316Z-h2-powerloss-after-payload-staged-mpv-stuck-135f397` bloqueia esse alvo: MPV ficou em uma midia enquanto o status avancou. Para o alvo `9bebaf1`, o preflight `docs/evidence/c18-update-validation/20260618T024300Z-h2-powerloss-board-preflight-after-topology-prep-p0-9bebaf1/` precedeu a sessao fisica, e o P0 seletivo do piloto (`pilot_powerloss_p0`) passou nos 5 checkpoints em `docs/evidence/c18-update-validation/20260618T034601Z-pilot-p0-powerloss-9bebaf1/`. A validacao target-current final passou, a topologia foi restaurada para M6, e o preflight H2 power-loss corrente foi renovado em `docs/evidence/c18-update-validation/20260618T095819Z-h2-powerloss-board-preflight-after-target-health-9bebaf1/`. Isso liberou piloto assistido e preparou a sessao H2; a matriz completa so foi fechada na rodada fisica de 2026-07-03. |
| Power-loss fisico 2026-07-03 | Nova sessao H2 acumulou 5 checkpoints adicionais verdes para `9bebaf1` (`after_payload_staged`, `after_release_dir_created`, `after_extract`, `after_state_verifying`, `after_health_passed`), totalizando 10/17 com os 5 P0 anteriores. A primeira tentativa em `after_release_tree_fsync` voltou para o baseline `m6-a` e falhou deep-health com status avancando enquanto MPV ficou em uma unica midia; evidencia historica em `docs/evidence/c18-update-validation/20260703T012000Z-h2-powerloss-after-release-tree-fsync-blocked-status-mpv-mismatch-9bebaf1/` e decisao de hold em `docs/evidence/c18-update-validation/20260703T013918Z-h2-powerloss-baseline-m6a-hold-9bebaf1/`. Os reruns controlados de `after_release_tree_fsync`, `after_marker_written`, `after_previous_symlink`, `after_state_success` e `before_stage_cleanup` passaram em `docs/evidence/c18-update-validation/20260703T023000Z-h2-powerloss-after-release-tree-fsync-9bebaf1/`, `docs/evidence/c18-update-validation/20260703T024900Z-h2-powerloss-after-marker-written-9bebaf1/`, `docs/evidence/c18-update-validation/20260703T030000Z-h2-powerloss-after-previous-symlink-9bebaf1/`, `docs/evidence/c18-update-validation/20260703T031500Z-h2-powerloss-after-state-success-9bebaf1/` e `docs/evidence/c18-update-validation/20260703T032400Z-h2-powerloss-before-stage-cleanup-9bebaf1/`, levando a matriz aceita para 15/17. A tentativa inicial de `rollback_after_identify_links` em `docs/evidence/c18-update-validation/20260703T035500Z-h2-powerloss-rollback-after-identify-links-blocked-status-mpv-mismatch-9bebaf1/` nao ficou verde: a adocao segura voltou para `m6-a`, mas deep-health falhou por desalinhamento status/MPV. Com a ponte rollback-safe, `rollback_after_identify_links` passou em `docs/evidence/c18-update-validation/20260703T063000Z-h2-powerloss-final-rollback-bridge-9bebaf1/rollback_after_identify_links/`; o ultimo checkpoint, `rollback_after_current_unlinked`, tambem passou em `docs/evidence/c18-update-validation/20260703T063000Z-h2-powerloss-final-rollback-bridge-9bebaf1/rollback_after_current_unlinked/`, provando fallback de imagem apos corte fisico. A matriz fisica H2 esta 17/17. |
| Target live 2026-07-03 | Apos o hold do H2, o alvo `9bebaf1` foi aplicado na placa por caminho lab/homologation governado, preservando `m6-a` como previous e mantendo apply/rollback/reconcile publicos congelados com `rc=44`. O servico adotou o alvo, passou deep-health de 180s, e depois de reboot fisico passou deep-health adicional de 60s. Evidencia em `docs/evidence/c18-update-validation/20260703T015735Z-target-9bebaf1-live-apply-and-health/`. Isso prova saude curta do alvo em uso real, mas nao conta como checkpoint H2, soak, stable, producao ou thaw publico. |
| Estado vivo apos cortes interrompidos 2026-07-03 | Observacao read-only posterior mostrou a placa viva com `current` e `previous` ambos em `c18.player-runtime-m6-a-20260610T0501Z-29ff33b`; o alvo `9bebaf1` apareceu em quarentena por `physical_powerloss_trial`. O watchdog externo estava realinhando MPV repetidamente para recuperar a experiencia. Esse estado reforcou o hold: operacionalmente recuperavel, mas nao evidencia H2 limpa e nao contavel para os cortes recentes. A quarentena do alvo foi removida por reset lab-only governado em `docs/evidence/c18-update-validation/20260703T052900Z-target-quarantine-reset-after-interrupted-h2-9bebaf1/`, preservando freeze publico `rc=44`. |
| Topologia rollback-safe 2026-07-03 | Foi criado o pacote bridge `c18.player-runtime-homolog-20260703-baseline-bridge-8ac1c63`, `channel=homologation`, com `lab_variant=baseline_bridge`, release gate verde e arvore distinta do alvo. A placa foi preparada em `docs/evidence/c18-update-validation/20260703T054002Z-rollback-safe-bridge-topology-resume-9bebaf1/`: `current=9bebaf1`, `previous=baseline-bridge`, adoption verde e health verde para bridge e alvo final, sem watchdog recovery durante as janelas. Isso fecha a estrategia de `previous` rollback-safe para retomar os checkpoints restantes; nao e power-loss, soak, stable, producao ou thaw. |
...
| Rastreabilidade dos gates | Release gate decisivo git-guarda os diretorios coldboot/data; H1 atual `20260612T194911Z-1x-h1-decisive-traceability-refresh-7e40e80`; para o alvo `9bebaf1`, o snapshot H2 antigo `docs/evidence/c18-update-validation/20260618T101740Z-current-h2-readiness-head-0401375-9bebaf1/h2-readiness.json` continua historico e vermelho por power-loss 17/17, soak 24h, stable promotion e decisao formal de thaw. Apos `a4745b4`, o release gate repo passou limpo e a proxima foto H2/macro deve ser regenerada antes de qualquer claim atual. |
| Playback usuario | O alvo `9bebaf1` tem evidencia funcional curta e 10 minutos em placa real; `docs/evidence/c18-update-validation/user-level-10min-20260617T215450Z-9bebaf1/` passou apos calibracao de IPC, com 9 midias observadas, sem restart, sem falha de render e sem padrao de midia presa. Em 2026-07-03, o baseline rollback `m6-a` ainda mostrou risco pratico de status/MPV desalinhado. O watchdog externo passou a tentar realinhar MPV por IPC antes de restart, e o deep-health agora falha se qualquer recuperacao do watchdog ocorrer durante a janela; evidencias `docs/evidence/c18-update-validation/20260703T045300Z-status-mpv-watchdog-realign-guarded-lab-overlay/` e `docs/evidence/c18-update-validation/20260703T050600Z-health-journal-counter-smoke-lab-overlay/`. Isso melhora governanca/observabilidade, mas nao prova H2 nem elimina o bloqueio funcional do rollback antigo. |
...
| Proximo gate H2 | Com power-loss fisico 17/17 fechado, regenerar H2/pre-soak atuais e rodar soak/endurance 24h. Depois entram promocao stable e decisao final antes de qualquer caminho `stable`/producao. |
| Ainda nao provado | public thaw, GitHub/auto-pull, `stable`, producao e soak/endurance 24h |
