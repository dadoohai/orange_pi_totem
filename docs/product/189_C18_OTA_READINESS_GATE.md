...
| H1.5 homologation pilot | Caminho intermediario controlado por `scripts/qa/c18_player_runtime_pilot_readiness_gate.py`: `channel=homologation`, `ring=pilot`, entrega assistida, autorizacao formal, preflight com public freeze `rc=44`, pacote alvo e P0 power-loss seletivo. A evidencia `20260616T232546Z-current-pilot-readiness-5b2128c`, com `20260612T195516Z-pilot-readiness-traceability-refresh-c16fb3e`, fica preservada como historico; apos a evidencia negativa `docs/evidence/c18-update-validation/20260617T174316Z-h2-powerloss-after-payload-staged-mpv-stuck-135f397/`, `c16fb3e` nao e mais RC corrente aceitavel para piloto. |
| Evidencia 1u | offline `20260608T024500Z-1u-offline-build`, cold-boot HW `20260608T035330Z-1u-coldboot-deep-health` |
| Power-loss fisico | P0 seletivo historico do pacote `c16fb3e` cobria 5/17 (`after_current_symlink`, `rollback_after_current_to_previous`, `rollback_after_previous_removed`, `rollback_after_quarantine`, `rollback_after_state_success`), mas a tentativa fisica `after_payload_staged` preservada em `20260617T174316Z-h2-powerloss-after-payload-staged-mpv-stuck-135f397` bloqueia esse alvo: MPV ficou em uma midia enquanto o status avancou. Novo alvo corrigido precisa reiniciar a trilha de evidencia limpa; planner H2 `20260617T020912Z-h2-powerloss-matrix-plan-custom-setup-c16fb3e` fica historico. |
| Server-side/signature | Familia atual do pacote `c16fb3e` esta verde em `20260617T001804Z-server-side-current-c16fb3e`; gate offline valida artefatos reais, assinaturas/trust-anchor, audit log, auto-pull off, allowlist/staged rollout e component binding, sem publicar release |
| Rastreabilidade dos gates | Release gate decisivo git-guarda os diretorios coldboot/data; H1 atual `20260612T194911Z-1x-h1-decisive-traceability-refresh-7e40e80`; H2 readiness exige arvore limpa, inputs rastreados e separa hash H1 de release-gate do pacote, snapshot atual `20260617T030214Z-current-h2-readiness-13d4cbd` |
| Gate macro pre-H2 | `scripts/qa/c18_ota_macro_governance_gate.py` agrega H1, pilot readiness, H2 vermelho e diagnostico read-only em snapshot governado; nao substitui H2, nao reabre janela expirada e nao autoriza producao |
