...
| Proximo gate H2 | Com a topologia bridge preparada, recoletar preflight H2 fresco e completar os 2 checkpoints restantes de power-loss/torn-write (`rollback_after_identify_links` usando bridge como `previous`, e `rollback_after_current_unlinked` como fallback de imagem). Depois rodar soak/endurance 24h, promocao stable e decisao final antes de qualquer caminho `stable`/producao. |
