...
| Server-side publish gate / signature / auto-pull | **GATE HARDENED (off-board)**: evidence gate now rejects boolean-only claims and requires artifact-bound manifest/payload/release-gate files, per-asset attestation proofs, channel, auto-pull, allowlist, staged rollout, rollback and audit log structure. Real server-side evidence is still absent. | H2 production evidence |
...
- server-side publish governance gate / signature / auto-pull hardening:
  ...
  e log de auditoria. Ele nao publica release, nao liga auto-pull e nao promove
  stable.
...
| server-side/publish | Gate offline endurecido: evidencia fraca/booleans nao basta; exige artefatos reais hash-bound, provas de attestation, canais, auto-pull off, allowlist, staged rollout, rollback e auditoria | Evidencia real de publish/signature/attestation a ser produzida antes do H2 |
