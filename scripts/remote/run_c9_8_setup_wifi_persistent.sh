...
REMOTE_DIR="/tmp/dadooh-c9-8"
REMOTE_OUT_DIR="/tmp/dadooh-c9-8-setup-wifi-persistent"
REMOTE_WIZARD_OUT_DIR="/tmp/dadooh-c9-8-setup-wifi-persistent-wizard"
PROFILE_NAME="dadooh-c9-8-wifi-persistent"
CONFIRM_PHRASE="CONFIRMO C9.8 SETUP WIFI COM PAUSA DO PLAYER"

usage() {
  cat <<'USAGE'
Usage:
  run_c9_8_setup_wifi_persistent.sh [host] [--prepare-only|--run-cancel|--run-wifi-rollback-test|--run-wifi-persistent|--run-complete-with-wifi-persistent] [--tty N] [--timeout-sec N]

Modes:
  --prepare-only
      Copy C9.8 inputs to /tmp and run local/remote self checks only.
  ...
  --run-wifi-rollback-test
      Requires exact human confirmation. Runs the C9.7 rollback Wi-Fi path.

  --run-wifi-persistent
      Requires exact human confirmation. Pauses player, opens wizard on HDMI,
      and lets the operator configure persistent dedicated Wi-Fi.

  --run-complete-with-wifi-persistent
      Same as --run-wifi-persistent; intended acceptance mode for C9.8.
USAGE
}
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --run-cancel)
      ...
      ;;
    --run-wifi-rollback-test)
      MODE="run-wifi-rollback-test"
      ;;
    --run-wifi-persistent)
      MODE="run-wifi-persistent"
      ;;
    --run-complete-with-wifi-persistent)
...
case "$MODE" in
  prepare-only|run-cancel|run-wifi-rollback-test|run-wifi-persistent|run-complete-with-wifi-persistent)
    ;;
  *)
...
echo "Preparing C9.8 setup Wi-Fi workspace on $HOST"
ssh "$HOST" "umask 077 && mkdir -p '$REMOTE_DIR' '$REMOTE_OUT_DIR' && chmod 700 '$REMOTE_DIR' '$REMOTE_OUT_DIR'"

echo "Copying C9.8 inputs to temporary board workspace"
...
echo "Running C9.8 prepare checks on the board"
...
PERSISTENT_SCRIPTED_OUT="$REMOTE_OUT_DIR/scripted-wifi-persistent"
ALLOW_OUT="$REMOTE_OUT_DIR/validate-allow"
REAL_OUT="$REMOTE_OUT_DIR/validate-real"

umask 077
rm -rf "$SCRIPTED_OUT" "$PERSISTENT_SCRIPTED_OUT" "$ALLOW_OUT" "$REAL_OUT"
...
python3 "$WIZARD" \
  --scripted \
  --environment-id ENV-C9-8-SMOKE \
  --rotation-key landscape \
  --network-step wifi_real_test \
  --out-dir "$SCRIPTED_OUT" >/dev/null
python3 "$WIZARD" \
  --scripted \
  --environment-id ENV-C9-8-PERSISTENT-SMOKE \
  --rotation-key landscape \
  --network-step wifi_persistent \
  --out-dir "$PERSISTENT_SCRIPTED_OUT" >/dev/null
...
python3 - "$PERSISTENT_SCRIPTED_OUT/setup-status.json" <<'PY'
import json
import pathlib
import sys
status = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
network = status.get("network", {})
assert network.get("network_step") == "wifi_persistent"
assert network.get("wifi_activation_result") == "not_run"
assert network.get("dedicated_profile_persistent") is False
PY

echo "remote C9.8 prepare checks: ok"
REMOTE_PREP

if [ "$MODE" = "prepare-only" ]; then
  echo "C9.8 prepare-only complete. Evidence dir reserved: $REMOTE_OUT_DIR"
  exit 0
fi
...
echo "Before pausing the player and opening C9.8 setup, type exactly:"
echo "$CONFIRM_PHRASE"
printf '> '
...
echo "Running authorized C9.8 setup Wi-Fi integration on local console"
...
payload = {
    "schema_version": "dadooh-c9.8-setup-wifi-persistent-run.v1",
    "mode": mode,
    ...
    "dedicated_profile_persistent": bool(network.get("dedicated_profile_persistent", False)),
...
on_exit() {
  ...
  kill_c9_8_wizard_if_running || true
...
if [ "$EXPECTED_RESULT" = "cancelled" ]; then
  ...
  python3 - "$WIZARD_OUT/setup-status.json" "$MODE" <<'PY'
import json
import pathlib
import sys
status = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
mode = sys.argv[2]
network = status.get("network", {})
expected_step = "wifi_real_test" if mode == "run-wifi-rollback-test" else "wifi_persistent"
if network.get("network_step") != expected_step:
    raise SystemExit("network_step_not_expected")
if network.get("wifi_real_test_attempted") is not True:
    raise SystemExit("wifi_apply_not_attempted")
if network.get("wifi_activation_result") != "success":
    raise SystemExit("wifi_activation_not_success")
if expected_step == "wifi_real_test":
    if network.get("rollback_after_test") != "success":
        raise SystemExit("rollback_after_test_not_success")
    if network.get("dedicated_profile_persistent") is not False:
        raise SystemExit("rollback_test_persisted_profile")
else:
    if network.get("dedicated_profile_present_final") is not True:
        raise SystemExit("persistent_profile_not_present")
    if network.get("dedicated_profile_persistent") is not True:
        raise SystemExit("persistent_profile_not_recorded")
    if network.get("rollback_after_test") not in {"not_requested", "not_run"}:
        raise SystemExit("persistent_profile_unexpected_rollback")
...
echo "C9.8 setup Wi-Fi integration complete. Evidence: $REMOTE_OUT_DIR"
