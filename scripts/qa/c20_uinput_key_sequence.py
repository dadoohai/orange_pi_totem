...
def hold(device, key_code: int, *, hold_sec: float, delay_sec: float) -> None:
    ...
    while True:
        ...
        if remaining <= 0:
            break
        time.sleep(min(repeat_interval_sec, remaining))
        if time.monotonic() < deadline:
            emit_event(device, EV_KEY, key_code, 2)
            emit_event(device, EV_SYN, SYN_REPORT, 0)
    emit_event(device, EV_KEY, key_code, 0)
    emit_event(device, EV_SYN, SYN_REPORT, 0)
    time.sleep(delay_sec)
    ...
    *,
    ...
    with device_path.open("wb", buffering=0) as device:
        ...
        try:
            ...
            if hold_sec > 0:
                if len(keys) != 1:
                    raise ValueError("--hold-sec requires a single key")
                hold(device, keys[0], hold_sec=hold_sec, delay_sec=delay_sec)
            else:
                for key in keys:
...
def main() -> int:
    ...
    parser.add_argument("--hold-sec", type=float, default=0.0, help="Hold a single key for this many seconds.")
