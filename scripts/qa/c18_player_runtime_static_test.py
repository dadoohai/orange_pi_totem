...
class C18PlayerRuntimeStaticTest(unittest.TestCase):
    ...
    def test_c18_deriver_uses_governed_snapshot(self) -> None:
        ...
        self.assertIn('TAG = "c18-hwdecode-lab-1s"', derive)
        self.assertIn('VERSION = "c18.image-lab.1s"', derive)
        self.assertIn('MARKER = "/etc/dadooh/c18-hwdecode-lab-1s-image"', derive)
        self.assertIn('"round": "C18.IMAGE-LAB.1s"', derive)
        self.assertIn("=== C18.IMAGE-LAB.1s RESULT ===", derive)
        self.assertIn("1r + cold-boot/power-loss pre-hardening gates", derive)
