...
class C18PlayerRuntimeStaticTest(unittest.TestCase):
    ...
    def test_c18_deriver_uses_governed_snapshot(self) -> None:
        ...
        self.assertIn('TAG = "c18-hwdecode-lab-1q"', derive)
        self.assertIn('VERSION = "c18.image-lab.1q"', derive)
        self.assertIn('MARKER = "/etc/dadooh/c18-hwdecode-lab-1q-image"', derive)
        self.assertIn('"round": "C18.IMAGE-LAB.1q"', derive)
        self.assertIn("=== C18.IMAGE-LAB.1q RESULT ===", derive)
        self.assertIn("1p + privileged boot reconcile", derive)
