...
OUT_DIR="$BASE_DIR/wifi-snapshot-$TIMESTAMP"
ARCHIVE="$BASE_DIR/wifi-snapshot-$TIMESTAMP.tar.gz"
...
if command -v tar >/dev/null 2>&1; then
  tar -C "$BASE_DIR" -czf "$ARCHIVE" "wifi-snapshot-$TIMESTAMP"
  tar_rc=$?
  if [ "$tar_rc" -eq 0 ]; then
    echo "Wi-Fi snapshot directory: $OUT_DIR"
    echo "Archive: $ARCHIVE"
    echo "WARNING: raw artifacts may contain SSIDs, IPs and connection names."
  else
  ...
  echo "ERROR: tar not found; Wi-Fi snapshot kept at $OUT_DIR" >&2
  exit 127
fi
