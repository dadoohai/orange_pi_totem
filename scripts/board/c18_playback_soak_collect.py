...
def cycle_brief(index: int, cycle_dir: Path, result: dict[str, Any]) -> dict[str, Any]:
...
def summarize(cycles: list[dict[str, Any]], *, output_dir: Path, started_at: str, ended_at: str) -> dict[str, Any]:
...
def collect_cycle(args: argparse.Namespace, output_dir: Path, index: int) -> dict[str, Any]:
    ...
    return cycle_brief(index, cycle_dir, result)
...
def run_soak(args: argparse.Namespace) -> dict[str, Any]:
    ...
    ended_at = timestamp()
    summary = summarize(cycles, output_dir=output_dir, started_at=started_at, ended_at=ended_at)
...
def self_test() -> None:
    ...
    ok = summarize(cycles, output_dir=Path("/tmp/self-test"), started_at="a", ended_at="b")
    ...
    bad = summarize(bad_cycles, output_dir=Path("/tmp/self-test"), started_at="a", ended_at="b")
